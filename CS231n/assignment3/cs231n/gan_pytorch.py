...
def discriminator_loss(logits_real, logits_fake):
    ...
    label = torch.ones_like(logits_real).type(dtype)
    label = torch.cat((label, torch.zeros_like(logits_real).type(dtype))).squeeze()
    scores = torch.cat((logits_real, logits_fake))
    loss = bce_loss(scores, label) * 2
...
def generator_loss(logits_fake):
    ...
    label = torch.ones_like(logits_fake).type(dtype).squeeze()
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5 * torch.mean(torch.pow(scores_real - 1, 2)) + \
           0.5 * torch.mean(torch.pow(scores_fake, 2))
