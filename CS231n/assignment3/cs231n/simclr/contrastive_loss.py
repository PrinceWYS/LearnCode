...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2 * N):
            if j != k:
                exp_sum_1 += torch.exp(sim(z_k, out[j]) / tau)
        l_1 = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / exp_sum_1)

        exp_sum_2 = 0.0
        for j in range(2 * N):
            if j != k + N:
                exp_sum_2 += torch.exp(sim(z_k_N, out[j]) / tau)
        l_2 = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / exp_sum_2)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.matmul(out, out.T) / (torch.matmul(norm_out, norm_out.T))
