...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][1].shape[0]
    dx, dWx, dWh, db, step_dprevh =\
      [], np.zeros((D, H)), np.zeros((H, H)), np.zeros((H)), np.zeros((N, H))
    for t in range(T - 1, -1, -1):
        step_dx, step_dprevh, step_dWx, step_dWh, step_db =\
          rnn_step_backward(dh[:, t, :] + step_dprevh, cache[t])
        dx.append(step_dx)
...
def lstm_backward(dh, cache):
    ...
    step_dprevh, step_dprevc = np.zeros((N, H)), np.zeros((N, H))

    for t in range(T-1, -1, -1):
        step_dx, step_dprevh, step_dprevc, step_dWx, step_dWh, step_db = \
            lstm_step_backward(dh[:, t, :]+step_dprevh, step_dprevc, cache[t])
        dx.append(step_dx)
        dWx += step_dWx
        dWh += step_dWh
        db += step_db
